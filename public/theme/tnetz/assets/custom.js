...
function pgOrder(){
  ...
  }).catch(function(x){$('pc').innerHTML='<div class="empty"><div class="empty-ico">'+L('alert-circle',48)+'</div><div class="empty-txt">'+e(x.message)+'</div></div>';LI();});
}
window._pay=function(tn){api('/user/order/getPaymentMethod').then(function(j){var ms=j.data||[];if(!ms.length){toast('Chưa cấu hình thanh toán','err');return;}openModal(L('credit-card',20)+' Thanh toán',function(box){ms.forEach(function(m){var el=C('div');el.className='region';el.innerHTML='<div class="region-ico" style="background:var(--prBg)">'+L('credit-card',18)+'</div><div><div class="region-name">'+e(m.name)+'</div></div>';el.onclick=function(){el.innerHTML='<span class="spin"></span>';api('/user/order/checkout',{method:'POST',body:{trade_no:tn,method:m.id}}).then(function(j){if(j.data){closeModal();if(j.data.type===1&&j.data.data)window.open(j.data.data);else if(j.data.type===0)toast('✅ Thanh toán thành công');else if(j.data.type===-1)window.location.href=j.data.data;pgOrder();}else toast(j.message||'Lỗi','err');}).catch(function(x){toast(x.message,'err');});};box.appendChild(el);});LI();});}).catch(function(x){toast(x.message,'err');});};
...
/* ══════════ INVITE ══════════ */
function pgInvite(){shell('Giới thiệu',ld());Promise.all([api('/user/invite/fetch'),api('/user/invite/details'),api('/user/info')]).then(function(r){var inv=r[0].data||{},det=r[1].data||[],codes=inv.codes||[],ui=r[2].data||{};
  var h='<div class="stats" style="grid-template-columns:1fr 1fr 1fr 1fr;margin-bottom:20px"><div class="stat-card"><div class="s-ico" style="background:var(--grad1)">'+L('wallet',18)+'</div><div class="s-val">'+fM(ui.commission_balance||0)+'</div><div class="s-lbl">Hoa hồng khả dụng</div></div><div class="stat-card"><div class="s-ico" style="background:var(--grad3)">'+L('clock',18)+'</div><div class="s-val">'+fM(inv.stat?inv.stat[0]:0)+'</div><div class="s-lbl">Hoa hồng chờ</div></div><div class="stat-card"><div class="s-ico" style="background:var(--grad4)">'+L('check-circle',18)+'</div><div class="s-val">'+fM(inv.stat?inv.stat[1]:0)+'</div><div class="s-lbl">Đã nhận</div></div><div class="stat-card"><div class="s-ico" style="background:var(--grad2)">'+L('users',18)+'</div><div class="s-val">'+(inv.stat?inv.stat[2]:0)+'</div><div class="s-lbl">Đã giới thiệu</div></div></div>';
  // Transfer commission
  if((ui.commission_balance||0)>0)h+='<div class="card" style="margin-bottom:16px"><div class="card-h">'+L('arrow-right-left',16)+' Rút hoa hồng</div><div class="card-b"><div style="font-size:12px;color:var(--tx2);margin-bottom:10px">Chuyển hoa hồng vào số dư tài khoản</div><div style="display:flex;gap:8px"><input class="inp" id="tfAmt" type="number" placeholder="Nhập số tiền (×100)" style="flex:1"><button class="btn btn-p" id="tfBtn">'+L('send',14)+' Chuyển</button></div></div></div>';
  h+='<div class="card" style="margin-bottom:16px"><div class="card-h">'+L('ticket',16)+' Mã giới thiệu <button class="btn btn-p btn-sm" id="genC" style="margin-left:auto">'+L('plus',14)+' Tạo mã</button></div><div class="card-b">';
  if(codes.length){codes.forEach(function(c){var lk=location.origin+'/#/register?code='+c.code;h+='<div style="display:flex;align-items:center;gap:10px;padding:10px 0;border-bottom:1px solid var(--bdr2)"><span class="mono" style="flex:1">'+e(c.code)+'</span><button class="btn btn-sm" onclick="('+clip.toString()+')(\''+lk+'\')">'+L('copy',14)+' Link</button></div>';});}else h+='<div class="empty" style="padding:24px"><div class="empty-txt">Chưa có mã</div></div>';
  h+='</div></div>';
  if(det.length){h+='<div class="card"><div class="card-h">'+L('history',16)+' Lịch sử hoa hồng</div><div class="card-b" style="overflow-x:auto"><table class="tbl"><thead><tr><th>Đơn hàng</th><th>Hoa hồng</th><th>Trạng thái</th><th>Ngày</th></tr></thead><tbody>';det.forEach(function(d){h+='<tr><td class="mono">#'+d.order_id+'</td><td style="font-weight:700">'+fM(d.get_amount)+'</td><td><span class="tag '+(d.status?'tag-ok':'tag-warn')+'">'+(d.status?'Đã nhận':'Chờ')+'</span></td><td style="font-size:12px">'+fDT(d.created_at)+'</td></tr>';});h+='</tbody></table></div></div>';}
  $('pc').innerHTML=h;$('pc').classList.add('fade-in');LI();
  $('genC').onclick=function(){api('/user/invite/save').then(function(){toast('✅');pgInvite();}).catch(function(x){toast(x.message,'err');});};
  var tfB=$('tfBtn');if(tfB)tfB.onclick=function(){var amt=parseInt($('tfAmt').value);if(!amt||amt<=0){toast('Nhập số tiền hợp lệ','err');return;}api('/user/transfer',{method:'POST',body:{transfer_amount:amt}}).then(function(){toast('✅ Đã chuyển');pgInvite();}).catch(function(x){toast(x.message,'err');});};
}).catch(function(x){$('pc').innerHTML='<div class="empty"><div class="empty-ico">'+L('alert-circle',48)+'</div><div class="empty-txt">'+e(x.message)+'</div></div>';LI();});}

/* ══════════ TICKET ══════════ */
function pgTicket(){shell('Hỗ trợ',ld());api('/user/ticket/fetch').then(function(j){var ts=j.data||[];var h='<div style="margin-bottom:16px"><button class="btn btn-grad" id="newT">'+L('edit',16)+' Tạo ticket mới</button></div>';if(!ts.length)h+='<div class="empty"><div class="empty-ico">'+L('message-circle',48)+'</div><div class="empty-txt">Chưa có ticket</div></div>';else{var st={0:['Đang mở','tag-ok'],1:['Đã đóng','tag-err']};ts.forEach(function(t){var s=st[t.status]||['—',''];h+='<div class="card" style="margin-bottom:10px;cursor:pointer" onclick="window._vt('+t.id+')"><div class="card-b" style="display:flex;align-items:center;gap:12px"><div style="flex:1"><div style="font-weight:600">'+e(t.subject)+'</div><div style="font-size:12px;color:var(--tx3);margin-top:2px">'+fDT(t.created_at)+'</div></div><span class="tag '+s[1]+'">'+s[0]+'</span></div></div>';});}
  $('pc').innerHTML=h;$('pc').classList.add('fade-in');LI();
  $('newT').onclick=function(){openModal(L('edit',20)+' Ticket mới',function(box){box.innerHTML+='<div class="inp-group"><label class="inp-lbl">Tiêu đề</label><input class="inp" id="tS"></div><div class="inp-group"><label class="inp-lbl">Nội dung</label><textarea class="inp" id="tC" rows="4"></textarea></div><button class="btn btn-grad btn-block" id="tB">Gửi ticket</button>';setTimeout(function(){$('tB').onclick=function(){var s=$('tS').value.trim(),c=$('tC').value.trim();if(!s||!c){toast('Nhập đầy đủ','err');return;}api('/user/ticket/save',{method:'POST',body:{subject:s,message:c,level:0}}).then(function(){toast('✅');closeModal();pgTicket();}).catch(function(x){toast(x.message,'err');});};},50);LI();});};
}).catch(function(x){$('pc').innerHTML='<div class="empty"><div class="empty-ico">'+L('alert-circle',48)+'</div><div class="empty-txt">'+e(x.message)+'</div></div>';LI();});}
window._vt=function(id){openModal(L('message-circle',20)+' Ticket #'+id,function(box){box.innerHTML=ld();api('/user/ticket/fetch?id='+id).then(function(j){var t=j.data;if(!t){box.innerHTML='<p>Không tìm thấy</p>';return;}var ms='';(t.message||[]).forEach(function(m){ms+='<div style="margin-bottom:10px;padding:12px 16px;border-radius:12px;background:'+(m.is_me?'var(--prBg)':'var(--bgH)')+';'+(m.is_me?'margin-left:24px':'margin-right:24px')+'"><div style="font-size:11px;color:var(--tx3);margin-bottom:4px">'+(m.is_me?'Bạn':'Hỗ trợ')+' · '+fDT(m.created_at)+'</div><div style="font-size:13px">'+e(m.message)+'</div></div>';});box.innerHTML='<div style="font-weight:700;margin-bottom:14px">'+e(t.subject||'')+'</div><div style="max-height:320px;overflow-y:auto;margin-bottom:16px">'+ms+'</div>'+(t.status===0?'<div class="inp-group"><textarea class="inp" id="tR" rows="2" placeholder="Trả lời..."></textarea></div><div class="btn-group"><button class="btn btn-p" id="tRB">'+L('send',14)+' Gửi</button><button class="btn btn-d btn-sm" id="tCB">Đóng ticket</button></div>':'<div style="text-align:center;color:var(--tx3);font-size:13px;padding:12px">Ticket đã đóng</div>');if(t.status===0)setTimeout(function(){$('tRB').onclick=function(){var m=$('tR').value.trim();if(!m)return;api('/user/ticket/reply',{method:'POST',body:{id:id,message:m}}).then(function(){toast('✅');window._vt(id);}).catch(function(x){toast(x.message,'err');});};$('tCB').onclick=function(){api('/user/ticket/close',{method:'POST',body:{id:id}}).then(function(){toast('✅');closeModal();pgTicket();}).catch(function(x){toast(x.message,'err');});};},50);LI();}).catch(function(x){box.innerHTML='<p style="color:var(--err)">'+x.message+'</p>';});});};

/* ══════════ KNOWLEDGE ══════════ */
function pgKnowledge(){shell('Hướng dẫn',ld());api('/user/knowledge/fetch').then(function(j){var items=j.data||[];if(!items.length){$('pc').innerHTML='<div class="empty"><div class="empty-ico">'+L('book-open',48)+'</div><div class="empty-txt">Chưa có hướng dẫn</div></div>';LI();return;}var h='';items.forEach(function(item){h+='<div class="card" style="margin-bottom:10px;cursor:pointer"><div class="card-b"><div style="display:flex;align-items:center;gap:10px"><span style="width:36px;height:36px;border-radius:10px;background:var(--prBg);display:flex;align-items:center;justify-content:center;flex-shrink:0;color:var(--pr)">'+L('file-text',18)+'</span><div style="flex:1"><div style="font-weight:600;font-size:14px">'+e(item.title)+'</div>'+(item.category?'<div style="font-size:11px;color:var(--tx3);margin-top:2px">'+e(item.category.name||'')+'</div>':'')+'</div><span style="color:var(--tx3);transition:transform .2s" class="kb-arrow">'+L('chevron-right',16)+'</span></div><div class="kb-body" style="display:none;margin-top:14px;padding-top:14px;border-top:1px solid var(--bdr2);font-size:13px;line-height:1.75">'+(item.body||'')+'</div></div></div>';});$('pc').innerHTML=h;$('pc').classList.add('fade-in');LI();$('pc').querySelectorAll('.card').forEach(function(card){card.onclick=function(){var body=card.querySelector('.kb-body'),arrow=card.querySelector('.kb-arrow');if(body.style.display==='none'){body.style.display='block';arrow.innerHTML=L('chevron-down',16);LI();}else{body.style.display='none';arrow.innerHTML=L('chevron-right',16);LI();}};});}).catch(function(x){$('pc').innerHTML='<div class="empty"><div class="empty-ico">'+L('alert-circle',48)+'</div><div class="empty-txt">'+e(x.message)+'</div></div>';LI();});}

/* ══════════ PROFILE ══════════ */
function pgProfile(){shell('Tài khoản',ld());
  Promise.all([api('/user/getSubscribe'),api('/user/info')]).then(function(r){
    user=r[0].data;var ui=r[1].data||{};var used=(user.u||0)+(user.d||0);
    $('pc').innerHTML='<div class="g g2">'+
    '<div>'+
      // Account info card
      '<div class="card" style="margin-bottom:16px"><div class="card-h">'+L('user',16)+' Thông tin tài khoản</div><div class="card-b">'+
        ir('Email',user.email)+ir('UUID','<span class="mono" style="font-size:10px">'+(user.uuid||'—')+'</span>',true)+
        ir('Gói',user.plan?user.plan.name:'Chưa có gói')+ir('Hết hạn',fD(user.expired_at))+
        ir('Data',fB(used)+' / '+fB(user.transfer_enable))+
        ir('Thiết bị',(user.alive_ip||0)+' / '+(user.device_limit||'∞'))+
        ir('Số dư',fM(ui.balance||0))+
        ir('Hoa hồng',fM(ui.commission_balance||0))+
        ir('Telegram',ui.telegram_id?'✅ Đã liên kết (#'+ui.telegram_id+')':'Chưa liên kết')+
        ir('Đăng ký',fDT(ui.created_at))+
      '</div></div>'+
      // Traffic stats card
      '<div class="card" style="margin-bottom:16px"><div class="card-h">'+L('bar-chart-3',16)+' Thống kê lưu lượng <button class="btn btn-sm" id="viewTraffic" style="margin-left:auto">Xem chi tiết</button></div><div class="card-b">'+
        '<div class="progress-wrap"><div class="progress-bar"><div class="progress-fill" style="width:'+Math.min(100,Math.round(used/(user.transfer_enable||1)*100))+'%"></div></div>'+
        '<div class="progress-info"><span>'+L('arrow-up',13)+' '+fB(user.u||0)+'</span><span>'+L('arrow-down',13)+' '+fB(user.d||0)+'</span></div></div>'+
        (user.reset_day!==null?'<div style="margin-top:10px;font-size:12px;color:var(--tx2)">'+L('refresh-cw',13)+' Đặt lại sau: <strong>'+user.reset_day+' ngày</strong>'+(user.allow_new_period?' <button class="btn btn-sm" id="newPeriodBtn" style="margin-left:6px">Kích hoạt sớm</button>':'')+'</div>':'')+
      '</div></div>'+
    '</div>'+
    '<div>'+
      // Password card
      '<div class="card" style="margin-bottom:16px"><div class="card-h">'+L('lock',16)+' Đổi mật khẩu</div><div class="card-b"><div class="inp-group"><label class="inp-lbl">Mật khẩu cũ</label><input class="inp" type="password" id="oP"></div><div class="inp-group"><label class="inp-lbl">Mật khẩu mới</label><input class="inp" type="password" id="nP"></div><button class="btn btn-p" id="cpB">'+L('save',14)+' Đổi mật khẩu</button></div></div>'+
      // Gift card
      '<div class="card" style="margin-bottom:16px"><div class="card-h">'+L('gift',16)+' Gift Card</div><div class="card-b"><div style="display:flex;gap:8px"><input class="inp" id="gcI" placeholder="Nhập mã gift card"><button class="btn btn-p" id="gcB">'+L('check',14)+' Đổi</button></div></div></div>'+
      // Settings card
      '<div class="card" style="margin-bottom:16px"><div class="card-h">'+L('settings',16)+' Cài đặt thông báo</div><div class="card-b">'+
        togRow('remind_expire','Nhắc hết hạn',ui.remind_expire)+
        togRow('remind_traffic','Nhắc hết lưu lượng',ui.remind_traffic)+
        togRow('auto_renewal','Tự động gia hạn',ui.auto_renewal)+
      '</div></div>'+
      // Actions card
      '<div class="card"><div class="card-h">'+L('shield',16)+' Bảo mật & Hành động</div><div class="card-b"><div class="btn-group" style="flex-wrap:wrap">'+
        '<button class="btn" id="rstB">'+L('refresh-cw',14)+' Reset link</button>'+
        '<button class="btn" onclick="window._ss()">'+L('monitor',14)+' Phiên đăng nhập</button>'+
        '<button class="btn" onclick="window._loginLog()">'+L('scroll-text',14)+' Lịch sử login</button>'+
        (ui.telegram_id?'<button class="btn btn-d" id="tgUnbind">'+L('unlink',14)+' Hủy Telegram</button>':'')+
      '</div></div></div>'+
    '</div></div>';
    $('pc').classList.add('fade-in');LI();
    // Event handlers
    ...
    $('gcB').onclick=function(){var c=$('gcI').value.trim();if(!c)return;api('/user/redeemgiftcard',{method:'POST',body:{giftcard:c}}).then(function(j){toast('✅ Thành công!');$('gcI').value='';user=null;pgProfile();}).catch(function(x){toast(x.message,'err');});};
    $('rstB').onclick=function(){if(!confirm('Reset link? Link cũ sẽ mất hiệu lực.'))return;api('/user/resetSecurity').then(function(){toast('✅ Đã reset');subData=null;user=null;}).catch(function(x){toast(x.message,'err');});};
    var npB=$('newPeriodBtn');if(npB)npB.onclick=function(){if(!confirm('Đặt lại lưu lượng sớm? Ngày hết hạn sẽ bị trừ tương ứng.'))return;api('/user/newPeriod',{method:'POST'}).then(function(){toast('✅ Đã đặt lại');user=null;pgProfile();}).catch(function(x){toast(x.message,'err');});};
    var tgU=$('tgUnbind');if(tgU)tgU.onclick=function(){if(!confirm('Hủy liên kết Telegram?'))return;api('/user/unbindTelegram').then(function(){toast('✅');user=null;pgProfile();}).catch(function(x){toast(x.message,'err');});};
    $('viewTraffic').onclick=window._traffic;
    // Settings toggle handlers
    document.querySelectorAll('.tog-switch').forEach(function(sw){sw.onclick=function(){var k=sw.dataset.key;var val=sw.classList.contains('on')?0:1;var body={};body[k]=val;api('/user/update',{method:'POST',body:body}).then(function(){sw.classList.toggle('on');toast('✅');}).catch(function(x){toast(x.message,'err');});};});
  }).catch(function(x){$('pc').innerHTML='<div class="empty"><div class="empty-ico">'+L('alert-circle',48)+'</div><div class="empty-txt">'+e(x.message)+'</div></div>';LI();});
}
function ir(l,v,raw){return '<div style="display:flex;justify-content:space-between;padding:10px 0;border-bottom:1px solid var(--bdr2);font-size:13px"><span style="color:var(--tx2)">'+l+'</span><span style="font-weight:600;word-break:break-all;text-align:right;max-width:60%">'+(raw?v:e(v||''))+'</span></div>';}
function togRow(key,label,val){return '<div style="display:flex;align-items:center;justify-content:space-between;padding:10px 0;border-bottom:1px solid var(--bdr2)"><span style="font-size:13px;color:var(--tx2)">'+label+'</span><div class="tog-switch'+(val?' on':'')+'" data-key="'+key+'" style="width:42px;height:24px;border-radius:12px;background:'+(val?'var(--pr)':'var(--bdr)')+';cursor:pointer;position:relative;transition:all .2s"><div style="position:absolute;top:2px;'+(val?'right:2px':'left:2px')+';width:20px;height:20px;border-radius:50%;background:#fff;box-shadow:0 1px 3px rgba(0,0,0,.15);transition:all .2s"></div></div></div>';}

// Traffic stats modal
window._traffic=function(){openModal(L('bar-chart-3',20)+' Thống kê lưu lượng',function(box){box.innerHTML=ld();api('/user/stat/getTrafficLog').then(function(j){var logs=j.data||[];if(!logs.length){box.innerHTML='<div style="text-align:center;color:var(--tx3);padding:20px">Chưa có dữ liệu</div>';return;}var h='<div style="overflow-x:auto"><table class="tbl"><thead><tr><th>Ngày</th><th>'+L('arrow-up',13)+' Upload</th><th>'+L('arrow-down',13)+' Download</th><th>Tổng</th></tr></thead><tbody>';logs.forEach(function(l){h+='<tr><td style="font-size:12px">'+fD(l.record_at)+'</td><td>'+fB(l.u||0)+'</td><td>'+fB(l.d||0)+'</td><td style="font-weight:700">'+fB((l.u||0)+(l.d||0))+'</td></tr>';});h+='</tbody></table></div>';box.innerHTML=h;LI();}).catch(function(x){box.innerHTML='<p style="color:var(--err)">'+x.message+'</p>';});});};

// Login history modal
window._loginLog=function(){openModal(L('scroll-text',20)+' Lịch sử đăng nhập',function(box){box.innerHTML=ld();api('/user/stat/getLoginLog').then(function(j){var logs=j.data||[];if(!logs.length){box.innerHTML='<div style="text-align:center;color:var(--tx3);padding:20px">Chưa có lịch sử</div>';return;}var h='<div style="overflow-x:auto;max-height:400px;overflow-y:auto"><table class="tbl"><thead><tr><th>IP</th><th>UA</th><th>Thời gian</th></tr></thead><tbody>';logs.forEach(function(l){h+='<tr><td class="mono" style="font-size:11px">'+(l.ip||'—')+'</td><td style="font-size:11px;max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="'+e(l.ua||'')+'">'+e((l.ua||'').substring(0,40))+'</td><td style="font-size:11px">'+fDT(l.login_at)+'</td></tr>';});h+='</tbody></table></div>';box.innerHTML=h;}).catch(function(x){box.innerHTML='<p style="color:var(--err)">'+x.message+'</p>';});});};

// Active sessions modal
window._ss=function(){openModal(L('monitor',20)+' Phiên đăng nhập',function(box){box.innerHTML=ld();api('/user/getActiveSession').then(function(j){var ss=j.data||[];if(!ss.length){box.innerHTML='<div style="text-align:center;color:var(--tx3);padding:20px">Không có phiên nào</div>';return;}var h='';ss.forEach(function(s){h+='<div style="display:flex;align-items:center;gap:10px;padding:12px 0;border-bottom:1px solid var(--bdr2)"><div style="width:36px;height:36px;border-radius:10px;background:var(--bg3);display:flex;align-items:center;justify-content:center">'+L('monitor',16)+'</div><div style="flex:1"><div style="font-size:13px;font-weight:600">'+e(s.ip||'—')+'</div><div style="font-size:11px;color:var(--tx3)">'+fDT(s.created_at)+'</div></div><button class="btn btn-sm btn-d" onclick="window._rs(\''+s.session_id+'\')">'+L('trash-2',13)+'</button></div>';});box.innerHTML=h;LI();}).catch(function(x){box.innerHTML='<p style="color:var(--err)">'+x.message+'</p>';});});};
